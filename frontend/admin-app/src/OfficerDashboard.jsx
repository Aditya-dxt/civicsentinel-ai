...
function CitizenReportBoard({ events, loading, theme, isDark }) {
  ...
  useEffect(() => {
    if (events && events.length > 0) {
      ...
      fetch(`${API}/events`)
        .then(r => r.json())
        .then(d => setReports(Array.isArray(d) ? d : (d.events || [])))
        .catch(() => setReports([]))
  ...
  const cities = useMemo(() => {
    const s = new Set(reports.map(r => r.city || r.location).filter(Boolean));
    return ["all", ...Array.from(s).sort()];
  }, [reports]);

  const filtered = useMemo(() => {
    if (cityFilter === "all") return reports;
    return reports.filter(r => (r.city || r.location || "") === cityFilter);
  }, [reports, cityFilter]);

  const getStatus = (r, i) => {
    const key = r.id || r._id || String(i);
    return statuses[key] || r.status || "submitted";
  };

  const setStatus = (r, i, val) => {
    const key = r.id || r._id || String(i);
    setStatuses(prev => ({ ...prev, [key]: val }));
  };

  const STATUS_OPTS = [
    { value:"submitted",  label:"📤 Submitted",  color:"rgba(0,200,255,.8)" },
    { value:"in_process", label:"⏳ In Process",  color:"#ffb800" },
    { value:"approved",   label:"✅ Approved",    color:"#00ff9d" },
  ];

  const ISSUE_ICONS = { water:"💧", road:"🚧", electricity:"⚡", garbage:"🗑️", crime:"🚨", health:"🏥", encroachment:"🏗️", other:"📌" };
  ...
  if (loading || fetchLoading) return (
    <div style={{padding:"60px 0",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:14}}>
      <div style={{width:32,height:32,border:`2px solid ${theme.accent}22`,borderTopColor:theme.accent,borderRadius:"50%",animation:"spin .7s linear infinite"}}/>
      <span style={{fontSize:13,color:theme.txtMute,fontFamily:"'DM Mono',monospace"}}>Loading citizen reports...</span>
    </div>
  );
    ...
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:12}}>
        <div>
          ...
          <div style={{fontSize:12,color:theme.txtMute,marginTop:2}}>
            {filtered.length} report{filtered.length!==1?"s":""} {cityFilter!=="all"?`from ${cityFilter}`:"across all cities"} · click to expand
          </div>
        </div>
      ...
      {filtered.length === 0 ? (
        <div style={{padding:"60px 0",textAlign:"center",color:theme.txtMute,fontSize:13,fontFamily:"'DM Mono',monospace"}}>
          <div style={{fontSize:36,marginBottom:12}}>📭</div>
          No reports {cityFilter!=="all"?`from ${cityFilter}`:"yet"}. Submit complaints from the Citizen App.
        </div>
      ) : (
                ...
                <div onClick={() => setExpandedId(isExpanded ? null : rid)}
                  ...
                  <div style={{minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3,flexWrap:"wrap"}}>
                      ...
                      <span style={{fontSize:11,color:theme.txtMute}}>📍 {ward}</span>
                      <span style={{fontSize:11,color:theme.txtMute}}>🕐 {date}</span>
                      ...
                      <div>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px 20px",marginBottom:12}}>
                          {[
                            ...
                            ["📍 Location",    `${ward}, ${city}`],
                            ["🕐 Submitted",   date],
                            ["⚡ Risk Score",  risk > 0 ? `${risk}/100 — ${riskLabel(risk)}` : "Not scored"],
                            ["🆔 Report ID",   rid],
                            ["📊 Status",      statusOpt.label],
                          ].map(([label, val]) => (
                            <div key={label}>
                              <div style={{fontSize:10,color:theme.txtMute,marginBottom:3}}>{label}</div>
                              <div style={{fontSize:13,color:theme.txt,fontWeight:600}}>{val}</div>
                            </div>
                          ))}
                        </div>
                        <div>
